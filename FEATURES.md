...
| 53 | 🔴 `python <clone>/krokai …` — the exact command the install document prescribes — had **never worked**: directory-run gives `__main__.py` no parent package, and every earlier test ran `-m` from inside the clone, where the cwd quietly supplies what the documented command does not. Found by executing the document end-to-end in an empty folder | `__main__.py` bootstraps `sys.path`; the self-test runs the package directory from a foreign cwd |
| 54 | 🔴 `file_matches` compared numbers as substrings: part **245** claimed `8CFR-1245.2-EOIR.xml` and `part-245a`, `usc 255` matched inside `1255` — and the address layer then **blessed** the wrong file. In the sister project the same class showed a 7 KB EOIR twin as the source for the most-cited provision of a filing | number tokens in two name forms at once — flattening finds `PM6020199…`, separators tell 245 from 245a |
| 55 | The name rule for numbered policy memoranda had **never fired once**: the captured `602-0199` kept its dash, the flattened filename lost its own, and the two could not meet | both sides of a comparison get the same folding |
| 56 | 🔴 A truncated-but-alive PDF extraction **beat a complete one**: "fewer tokens = fewer split words" let 50 words win over 5 000. A single shared threshold was tried first and a control sat exactly on it | truncation measured in characters, splitting in tokens — two axes, not one number |
| 57 | Two different quotations sharing their first 60 characters read as **one banked entry**, so the second silently never reached the queue — regulatory prose is full of shared openings | `in_bank` compares the whole quotation, both sides through the same stripping |
| 58 | 🔴 Hyphen-grouped A-numbers and receipt numbers — the way real notices write them — walked through the outbound gate: rule and probe shared the same fused-spelling assumption, so the probe verified only itself. In the sister project this was a live leak to five vendors before it was a finding here | grouped forms in rule, probe and negative control |
| 59 | `uscis.com` — the exact agency name in a foreign zone, not a typo — walked past a lookalike detector that only knew the `gov`/`mil` labels | packs declare `official_domains`; a configured NAME worn as a whole label is flagged; the misspelling residual is printed, not closed |
| 60 | A correction applied to a memorandum body **missed the exhibit caption below it** — one file quoted the same provision both complete and truncated, and the truncated copy sat where the officer looks first | "one provision, two texts": clean and flagged quotations of the same address are paired in the report |
| 61 | The compact-surviving assistant block shipped as a template since 0.1.0 and **nothing in the install path ever placed it** — the piece that survives a `/compact` was exactly the piece nobody installed | `krokai init` writes it between markers; `doctor` reports it and the hooks, each with the exact command when absent |
| 62 | 🔴 The extraction chooser fired in the **wrong direction**: with a complete primary and a truncated ALTERNATE, "more tokens than the other side" read as "primary split words" and the truncated copy won. And its hair trigger (1.01×) let two line-break hyphens hand the round to a text missing a paragraph | truncation needs both axes to agree, tested in both directions; the split trigger is 1.15×, under the measured +51 % and above artefact noise |
| 63 | `write_claude_block` had three corruption vectors, each traced by a reviewer from the quoted code before any client paid: an orphaned marker deleted the user's text under the word "refreshed"; a non-UTF-8 file was transcoded wholesale to U+FFFD; a CRLF file flipped to LF end to end | unbalanced or duplicated markers and non-UTF-8 files are **refused with instructions**; line endings are preserved; the write is atomic |
| 64 | The `245a`-part veto only looked at token-initial tails, so `part245a` — no separator between the word and the number — walked past it; and the dashed memorandum needle had never met a flattened filename at all | digit-only lookbehind on the veto; both sides of every filename comparison get the same folding |
| 65 | 🔴🔴 The write-up of incident 58 illustrated it with the applicant's **real** A-number and receipt number, in four tracked files — and they were the POSITIVE FIXTURES, so the detector fired on them, the suite went green, and **the passing test was what kept the values in the file.** Published for ~6 hours; 0.5.0 withdrawn | one `FICTIONAL` table; a self-test that scans every file in the repo with the project's own detectors and allows only those VALUES — never a filename |
| 66 | 🔴🔴 The surname redaction **did not exist**. `README` promised it, `FEATURES.md` explained where to configure it, `name_patterns()` was correct — and `gate()` had no parameter for it and `config` had no key, so a brief with the client's full name passed and the gate printed `clean`. Every probe called `scan()` directly and supplied what the product did not | `surnames` threaded config → gate → prepare → run_round; the count is printed **including zero**; the regression test runs the real CLI end to end |
| 67 | 🔴🔴 `krokai quote` and `krokai check` disagreed on 5 of 6 realistic pastes. Not found-vs-missing: markdown residue turned `TRUNCATED_CONDITION` into `PUNCTUATION` — **the most dangerous verdict downgraded to a cosmetic one**. One normaliser, three pipelines | `check()` prepares the quotation itself, so no caller can forget |
| 68 | 🔴 `strip_markdown` deleted `* * *` — the CFR omitted-text marker — from the quote side, while the corpus side deliberately kept it. Half of an asymmetry that was reasoned about in writing, from one side only | markers held and restored verbatim; the control is that two ADJACENT bold spans still strip, which the obvious pattern breaks |
| 69 | 🔴 A source under 200 characters was dropped and the user told to **OCR a `.md` file**. The floor asks "did extraction fail?", which is a PDF question; a real short provision came back `NOT_FOUND`, this tool's fabrication signal. Found because my own test fixture fell under it twice in two rounds — a trap that catches the author of the tests twice is a defect in the tool | the floor applies only to extracted formats; short text sources are indexed AND reported, so a placeholder is still noticed |
| 70 | 🔴 A **filled** PDF form read as a blank one: values live in `/AcroForm /Fields` as `/V`, not in the page content. Measured in the sister project on the actually-filed I-485 — 765 fields, 455 filled, none extractable. And its thin text layer made the tool advise OCR on a file with no image | `read_pdf` appends `name = value` per filled field; empty fields are not invented; a form is no longer called a scan |
| 71 | 🔴 An exception's message and a channel's stderr reached the report cut to 200 characters — and this project's own docstring records that truncation is not a mask. `scrub()` looked like dead code because the port had dropped exactly these call sites | both scrubbed at the choke point; a probe assembles a key, raises it, and asserts it does not survive |
| 72 | `NOT_FOUND` meant two things and its explanation admitted it: *"not downloaded, or invented"*. 20 of 37 red items on a real filing were sources absent from a corpus of law by construction, and the real misses hid among them | `NO_SOURCE_ON_DISK` in a third bucket — never a pass; reachable only when the address resolves to nothing, so a cited-and-present source still reads `NOT_FOUND`, louder |
| 73 | 🔴🔴 **A DIFF THAT DELETES A SECRET STILL CONTAINS IT**, and the gate's own false positives were the camouflage. The review brief for this very release was refused twice: once on the key-shaped literals in `redact.py`'s removed lines, then on **fifteen** spans that genuinely were documented fictional values — at which point `--allow-pii` is the obvious move. Checking mechanically instead found **five spans it could not account for: the real A-number and receipt, on the removed lines of `selftest.py`.** The fifteen harmless matches hid the five real ones | no removed line matching ANY detector enters a payload, whatever the file; and an override is taken only after a mechanical check that every match is a value in `FICTIONAL` |
| 74 | 🔴🔴🔴 **THE MAIN GUARD WAS DEAD, AND THE HALF THAT WAS FIXED ON DAY ONE IS WHY.** `_bootstrap` forced UTF-8 on stderr and stdout from the project's first hour, with a docstring explaining that a mojibake warning is unreadable exactly when it matters. It never touched **stdin**. The harness sends UTF-8 bytes; `sys.stdin` decodes in the console code page, which is single-byte and therefore never raises — the JSON parses and every non-ASCII character silently becomes something else. Measured one variable at a time: UTF-8 in → exit 0 silent; the same bytes with `PYTHONIOENCODING` → exit 2 fires; **an ASCII path and an ASCII matter with curly quotes in the quotation → exit 0, silent.** The trigger is `U+201C`, which is what a model emits. The lesson generalises past encodings: **the visible half of a symmetric defect gets fixed immediately and the invisible half survives indefinitely** | read `sys.stdin.buffer` and decode explicitly; and `quote_guard` writes one log line per invocation, including the quiet ones, so that "nothing to report" and "dead" stop being the same observation |
| 75 | 🔴🔴 **The tool's own sidecar was indexed as primary law, and its warning header verified.** `krokai sidecar` writes into a sources directory, `Corpus` walks `.md`, and nothing in the file said what it was. `check()` returned a CLEAN verdict for *"Page breaks, signatures and exact layout exist only there"* — this package's own template — citing the sidecar as the source. The mechanism to prevent it already existed and had never been applied to the one generator that writes into the library | the sentinel goes in the sidecar header, in the CONTENT, never a rule about the name or extension; `bank_queue` passes the same sentinel, so the hook and `krokai check` stop grading against two different libraries |
| 76 | 🔴🔴 **The AcroForm reader added one release earlier put a certificate chain into the corpus.** Every govinfo PDF is signed; a `/Sig` field's `/V` is a *dictionary*, and `str()` of it is kilobytes of PKCS#7. Measured on a signed fixture: 4 267 characters, from which *"U.S. Government Printing Office, Washington"* came back **VERIFIED as law**. The shape to remember is that fixing a false negative by widening what gets indexed widens it too far unless something says stop | two independent tests — the field type, and a refusal to stringify any non-scalar — with the `Off`-in-a-text-field and unticked-checkbox controls both kept |
| 77 | 🔴 **A 40-page scan read as a document with a text layer.** The floor compared the whole document against 200 characters, so per-page furniture accumulated and the longer the scan the safer it looked: 7.8 characters a page over 40 pages passed as readable, and the biggest scan in a matter is the one the OCR list would not name | a per-page rate as well as a total, applied from three pages up so a genuine one-page proclamation is not condemned |
| 78 | 🔴 **The soft hyphen.** `U+00AD` renders as a hyphen only where a line breaks and as nothing elsewhere, so it is never part of the text and it defeats every comparison and every search invisibly. A correct quotation came back `PUNCTUATION` instead of `VERIFIED`; plain search for the word missed in the sidecar. A sister project measured 709 of them in one library, all at line ends, and its indexer deleted the character *before* joining — so there was nothing left to join and `applica`, `tion` entered its word list as words | the character and the line break are removed together, then the bare character; zero-width space, ZWNJ and ZWJ likewise. `strip_invisibles()` is shared with the sidecar writer, which needs the repair without the whitespace folding |
| 79 | 🔴 **A failed download could verify as law — the other side of incident 69.** Indexing short sources rescued a real 71-character savings clause and simultaneously indexed a scraped `404 Not Found` body, from which a phrase then **verified**. Both halves are true, so neither obvious answer is right: **length was only ever a proxy** for "the download failed", and a failed download says what it is | the test is on the content, in two tiers — strings only a server says fire anywhere, ordinary English like *access denied* only when it is essentially the whole file. The second tier exists because this project's own negative control caught the first draft firing on a due-process sentence about access being denied |
| 80 | 🔴 **A credential wrapped at a space walked through the gate.** The whole-text pass folded with no separator, which recovers a break *inside* a token and loses one *at a space*: the standard HTTP authorisation header with its scheme keyword at the end of one line and the credential at the start of the next folds without the space the pattern requires, so the gate printed `clean`. The reviewer who named the class offered four worked examples and **every one was wrong** — `-----BEGIN RSA \nPRIVATE KEY-----` still matches because `[A-Z ]{0,24}` absorbs `RSA`. Right finding, wrong proof, separated by running it | both joins are tried. The cost is stated rather than hidden: the empty join can fuse two unrelated lines into something key-shaped, and this class has no override — so a folded-only finding says it was folded and names both lines, because a gate the reader cannot act on is a gate that gets worked around |
| 81 | 🔴 **A row added to the library index landed below the table.** Markdown stops rendering a table at the first non-row line, so on a file that ends in prose the script printed "row added" — true, and useless. An invisible index entry is the same as none: the source is downloaded again, and its absence turns an honest quotation into a `NOT_FOUND` | inserted after the last table row, refused loudly when there is no table, written atomically |
| 82 | 🔴 **A query string destroyed a downloaded file's extension.** `title-8.xml?part=245` was saved as `title-8.xml-part-245`, which `read_any` does not recognise — so XML tags were never stripped and `&#xA7;` never unescaped, putting `S 103.2` in the corpus while every quotation says `§ 103.2`. Found by this module's own end-to-end run, which started producing quotations made of markup | the extension is a dispatch key, not decoration: the query goes in the stem |
| 83 | 🔴🔴 **A dated-edition marker bought an official stamp on any host.** The snapshot patterns are path and query fragments — `/annual/`, `?date=` — matched as substrings of the whole URL ahead of every host test, so `evil.com/historical/report.xml` was labelled **OFFICIAL BUT DATED**, downloaded with no `--allow-unknown-source`, and written into the library index with that phrase. 5 of 5 hostile URLs took the label. The second half of the substring bug fixed in 0.4.0: that round fixed the `primary` branch and left the branch that runs *first* | a pattern naming a host vouches for itself; a bare marker only qualifies a host already vouched |
| 84 | 🔴🔴 **The fix for a false negative deleted real law.** Moving the placeholder test out of the length branch let a 900-character bot wall be caught — and left tier 1 firing at every length, so ~10 000 characters of statute plus one scraped `Please enable JavaScript` footer excluded the whole document, and every correct quotation of it would read `NOT_FOUND`. The entire existing suite passed, because every control in it was short | fires anywhere in a document small enough to *be* an error page; above that it needs a second, distinct marker |
| 85 | 🔴 **A repeal is 22 characters, and the revision diff dropped everything under 25.** An operative sentence is short *because* it is operative — a fee, a deadline, an effective date. The floor was aimed at fragments left by splitting on the stop inside `U.S.C.` | a fragment is identified by having no internal space, not by being short |
| 86 | 🔴 **A relocated sentence read as no change at all.** A set loses order, so moving a conditioning sentence into another subsection reported `0 gone, 0 added, 100 % unchanged` — and the bank check agreed, because the words really are still in the file | `moved` is reported alongside, never folded into the similarity number |
| 87 | **A constant that cannot be varied from outside cannot be measured from outside.** The unwrap width was a local, so measuring what widening it costs set an attribute nothing read and returned an identical 0 findings at widths 3, 4, 5 and 6 — indistinguishable from a safe change | the width is module level, and the measurement that followed moved it 3 → 5 |
