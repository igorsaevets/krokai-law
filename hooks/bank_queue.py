...
The queue asks the question. The bank stores the answer. ``krokai close`` refuses to call the
...
def log(cfg, line):
    ...
    if cfg is None:
        return
    p = os.path.join(cfg.root, ".krokai", "bank_queue.log")
    try:
...
def main():
    ...
    from krokai.bank import candidates, read_bank, in_bank, append_queue
