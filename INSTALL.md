...
cd ~/tools/verbatim
python -m verbatim selftest        # expect N/N passed
```
...
```bash
python /wherever/you/put/verbatim/verbatim init .
python -m verbatim check          # from the matter folder, with the toolkit on PYTHONPATH
```

or, without touching `PYTHONPATH`:

```bash
python /wherever/you/put/verbatim/verbatim check --dir .
```
...
`python -m verbatim doctor` prints exactly which are present and what each absence means.
...
```bash
python -m verbatim install-hooks --dry-run     # see the diff
...
that installed them. Start a new session, then run `python -m verbatim doctor`.
...
```bash
python -m verbatim doctor      # ends with STATUS: READY
python -m verbatim check       # the whole-matter pass
python -m verbatim close       # end-of-session checks
```
...
```bash
python -m verbatim install-hooks --uninstall
```
