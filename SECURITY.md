...
server. There is no telemetry. `verbatim selftest` passes on a machine with no network at all, and
that is asserted by the suite rather than promised in prose.

The one exception is explicit and yours: `verbatim review` can hand a brief to a **separate**,
...
`verbatim/redact.py`, and the self-test will demand a probe line for any pattern you add — which is
