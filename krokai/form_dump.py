...
# 🔴 R77: every artifact this module writes is STAMPED as tool output. The dump folder is the
# user's to choose, and a `.forms.txt` or `cross-check.md` inside a sources directory would
# otherwise be indexed as a primary source - the sidecar incident of 0.6.x (a header of this
# package VERIFIED as law), through a generator added after that lesson was written down.
# R77 F-C minor: the SENTINEL import is DEFERRED into `_stamp()` so module load does not
# eagerly import `.run` (a future run→form_dump cycle would be silent otherwise — the
# fork-ageing lesson from R76 F5).


def _stamp(kind):
    """Return the tool-output stamp header for the given artifact `kind`: `"txt"` for a
    `.forms.txt` field dump, `"md"` for a Markdown report (cross-check, diff). Lazy sentinel
    import — see the module docstring."""
    from .run import SENTINEL
    if kind == "txt":
        return ("# %s : field dump written by `krokai form-dump`. NOT a primary source; "
                "values here must never verify as law.\n" % SENTINEL)
    return "<!-- %s : generated by `krokai form-dump`. Not a primary source. -->\n" % SENTINEL
